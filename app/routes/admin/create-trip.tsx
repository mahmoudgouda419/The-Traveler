...
export const loader = async () => {
    try {
        ...
        data = data.filter((country: any) => {
            const name = country.name?.common;
            return name !== "Israel" && name !== "Palestine" && name !== "Palestine, State of";
        });

        const formattedCountries = data.map((country: any) => ({
            name: `${country.flag || ""} ${country.name?.common || "Unknown"}`.trim(),
        ...
        formattedCountries.push({
            ...
            value: "Israel",
            openStreetMap: "https://www.openstreetmap.org/relation/1703884",
        });

        return formattedCountries.sort((a: Country, b: Country) => a.name.localeCompare(b.name));
...
const CreateTrip = ({ loaderData }: Route.ComponentProps) => {
    ...
    const mapData = formData.country
        ? [
            {
                country: formData.country,
                color: '#EA382E',
