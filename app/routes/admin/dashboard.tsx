...
export const clientLoader = async () => {
    ...
    const mappedUsers: UsersItineraryCount[] = allUsers.users.map((user) => ({
        ...
        name: user.name,
        count: trips.allTrips.filter(
            (trip) =>
                trip.userId === user.accountId &&
                new Date(trip.createdAt).getMonth() === new Date().getMonth() &&
                new Date(trip.createdAt).getFullYear() === new Date().getFullYear()
        ).length,
...
const Dashboard = ({ loaderData }: Route.ComponentProps) => {
    ...
    const displayedTrips = allTrips.slice(0, 4);
    const trips = displayedTrips.map((trip) => ({
    ...
    const usersAndTrips = [
        {
            title: 'Trip Activity This Month',
            dataSource: allUsers,
            field: 'count',
